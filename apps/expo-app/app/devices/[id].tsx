...
import { Button, H1, H2, Paragraph, XStack, YStack, Slider, Switch, Separator, ColorPicker, Spinner } from 'tamagui';
...
import { Device, Segment, State } from '../../state/devices';
...
function DeviceScreen() {
  ...
  if (!device) {
    return (
      <YStack f={1} padding="$4" backgroundColor="$background" justifyContent="center" alignItems="center">
        <Paragraph>Zařízení nenalezeno</Paragraph>
        <Button marginTop="$4" onPress={() => router.back()}>Zpět</Button>
      </YStack>
    );
  }
  ...
  const updateSegmentColor = (segmentId: number, colorIndex: number, color: [number, number, number]) => {
    const segmentIndex = deviceState.segments.findIndex(s => s.id === segmentId);
    if (segmentIndex !== -1) {
      devices$.deviceStates[id!].segments[segmentIndex].colors[colorIndex].set(color);
    }
  };
        ...
        <ScrollView>
          <YStack space="$4">
            ...
            {deviceState.segments.map((segment) => (
              <YStack key={segment.id} backgroundColor="$backgroundStrong" padding="$3" borderRadius="$2">
                <Paragraph fontWeight="bold">Segment {segment.id + 1}</Paragraph>
                <Paragraph size="$2">Rozsah: {segment.start} - {segment.stop}</Paragraph>
                
                <YStack marginTop="$2" space="$2">
                  {segment.colors.map((color, i) => (
                    <XStack key={i} space="$2" alignItems="center">
                      <Paragraph size="$2" width={60}>Barva {i + 1}:</Paragraph>
                      <XStack
                        width={30}
                        height={30}
                ...
                <YStack marginTop="$2">
                  ...
                  <Slider
                    ...
                    step={1}
                    onValueChange={([value]) => {
                      const segmentIndex = deviceState.segments.findIndex(s => s.id === segment.id);
                      if (segmentIndex !== -1) {
                        devices$.deviceStates[id!].segments[segmentIndex].speed.set(value);
                      }
                    }}
                  >
                    <Slider.Track>
