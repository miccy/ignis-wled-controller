...
import { Button, H1, Paragraph, XStack, YStack, Input, Separator, Form, Select } from 'tamagui';
...
function EffectsScreen() {
  ...
  return (
    <YStack f={1} padding="$4" backgroundColor="$background">
      ...
      {showAddForm && (
        <Form onSubmit={addEffect} marginBottom="$4" backgroundColor="$backgroundStrong" padding="$3" borderRadius="$2">
          <YStack space="$3">
            ...
            <Form.Field name="preset">
              ...
              <Form.Control asChild>
                <Select
                  value={selectedPreset !== null ? selectedPreset.toString() : undefined}
      ...
      {effectsList.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const presetName = presetOptions.find(p => p.value === item.presetId)?.name || 'Unknown';
              ...
              >
                <XStack justifyContent="space-between" alignItems="center">
                  <YStack>
                    ...
                    <Paragraph size="$2" opacity={0.6}>WLED efekt: {presetName}</Paragraph>
                  </YStack>
                  <XStack space="$2">
                    <Button
                      size="$3"
